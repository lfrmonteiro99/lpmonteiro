...
const skillIcons = {
  ...
  "Design Patterns": { color: "#61DAFB" },
  "SOLID Principles": { color: "#61DAFB" },
  JavaScript: { color: "#F7DF1E" },
  ReactJS: { color: "#61DAFB" },
  VueJS: { color: "#4FC08D" },
  TailwindCSS: { color: "#38B2AC" },
  ...
  PhpStorm: { color: "#000000" },
  PHPStan: { color: "#4F5B93" },
  "Unit Testing": { color: "#9B4F96" },
};

// Skill Badge Component
const SkillBadge = ({ skill }) => {
  const iconColor = skillIcons[skill]?.color || "#6B7280";
  ...
  return (
    <div className="rounded-full sm:w-fit w-full border text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 group/badge relative bg-gray-800/50 hover:bg-gray-700/80 text-gray-100 border-gray-600 flex items-center gap-2 py-2 px-3 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/20">
      <span className="transform group-hover/badge:scale-110 transition-transform duration-300">
        <svg
          className="hover:scale-125 transition-all ease-in-out duration-100 w-4 h-4"
...
const SkillCard = ({ category, skills }) => {
  const icon = categoryIcons[category] || {
    path: "M320 104.5c171.4 0 303.2 72.2 303.2 151.5S491.3 407.5 320 407.5c-171.4 0-303.2-72.2-303.2-151.5S148.7 104.5 320 104.5m0-16.8C143.3 87.7 0 163 0 256s143.3 168.3 320 168.3S640 349 640 256 496.7 87.7 320 87.7z",
    color: "#6B7280",
  };
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      ref={sectionRef}
      className="sm:min-h-screen sm:mx-auto pt-25 relative"
    >
      {/* Subtle background pattern */}
      <div className="absolute inset-0 bg-grid-pattern opacity-5"></div>

      <div className="container mx-auto py-4 px-4 sm:px-8 md:px-12 mt-16 md:mt-20 relative z-10">
        <h1 className="text-4xl font-bold text-white mb-8 text-center relative text-white">
          {`<Skills />`}
        </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {allSkills.map((skillCategory, index) => (
            <div
              key={index}
              className="skill-card min-h-[200px] opacity-0 translate-y-10 transition-all duration-500 ease-out"
